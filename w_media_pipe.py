"""
MediaPipe Hand Landmarker 기반 손 추적 (Windows 노트북 웹캠 버전)

실행:
    python w_media_pipe.py
종료:
    영상 창을 클릭한 뒤 'q' 키
"""

import os
os.environ['GLOG_minloglevel'] = '2'       # MediaPipe C++ 내부 경고 로그 숨기기
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'   # TensorFlow Lite 내부 로그 숨기기
...
CAMERA_INDEX = 0  # Windows 웹캠 인덱스 (내장 캠 0, USB 캠은 1 또는 2)
...
if not cap.isOpened():
    raise RuntimeError(
        ...
        "  2) CAMERA_INDEX 값을 1 또는 2로 변경 시도"
    )
