"""
OpenCV cv2.KalmanFilter 기반 객체 추적 (Windows 노트북 웹캠 버전)

원본: object_tracking_cv.py (Jetson + CSI 카메라 / GStreamer nvarguscamerasrc)
변경: Windows 내장/USB 웹캠 (DirectShow 백엔드)

w_object_tracking.py 는 Kalman Filter 를 numpy 로 직접 구현한 버전이고,
이 파일은 OpenCV 가 제공하는 cv2.KalmanFilter 를 쓰는 버전입니다.
상태 벡터에 속도(vx, vy)가 포함되어 가려짐(occlusion) 구간 예측이 더 자연스럽습니다.

파란 원  = 실제 검출된 객체 (measurement)
노란 원  = Kalman Filter 가 추정한 위치 (belief)

실행:
    python w_object_tracking_cv.py
종료:
    영상 창을 클릭한 뒤 'q' 키
"""
...
while True:
    ...
    cv2.imshow("Object Tracking (cv2.KalmanFilter)", frame)
